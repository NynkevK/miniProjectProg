...
class TicketEnglish(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.TicketViewEnglish(controller)

    def TicketViewEnglish(self, controller):
        ...
        self._informationLabel = tk.Label(frameLeft, text="Enter your licence plate\n for a ticket",
    ...
    def ProcessLicensePlate(self, entries):
        ...
        if str(licenseData.Fuel).lower().strip() == "diesel":
            if licenseData.DateFirstEntry.year >= 2001:
                ...
                self.ShowMessage("Your car is not allowed")
        else:
            self.HandleEntry(licensePlate)
    ...
    def HandleEntry(self, licensePlate):
        ...
        query = Database.Query(
            "SELECT Park_times.RecordGuid FROM Park_times INNER JOIN Car ON Car.RecordGuid = Park_times.ref_Car WHERE  Car.License_plate = ? AND Departure is null",
            licensePlate)
        if any(query):
            Database.Insert(
            ...
            Database.Insert(
                "INSERT INTO Park_times (Arrival,ref_Car) VALUES (GETDATE(),(SELECT RecordGuid FROM Car WHERE License_plate = ?))",
                licensePlate)

            self.ShowMessage("You are in! \n Welcome!")
            pass
